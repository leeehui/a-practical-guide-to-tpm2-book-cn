...
## 实体名称
实体的名称是TPM2.0的一个新概念，它主要用于解决TPM1.2规范中出现的问题。极端严苛的安全分析表明，攻击者有可能在数据被传送到TPM的过程中截获这些数据。TPM的设计可以阻止通过这种攻击篡改大部分的数据。但是因为TPM的存储资源有限，所以它允许一个密钥管理器在必要的时候将密钥加载到TPM或者由TPM导出。当密钥被加载到TPM之后，密钥就通过handle来访问。
## 总结
